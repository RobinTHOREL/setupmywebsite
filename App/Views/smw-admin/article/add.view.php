<?php
include(dirname(__DIR__).'/header.php');
include(dirname(__DIR__).'/menu_gauche.tpl.php');
?>

<div class="container">
    <div class="row"> <!-- exemple - ligne 1 -->
        <div class="col-10 title">
            <h2>Ajouter un article</h2>
        </div>
    </div>
    <div class="row"> <!-- exemple - ligne 2 -->
        <form action="" method="post">
            <div class="col-7">
                <input type="text" name="title" class="form-group col-4 col-offset-1" placeholder="Titre de l'article">
            </div> <!-- exemple - ligne 2 -->
            <div class="col-7">
                <textarea name="content" class="form-group"></textarea>
            </div>
            <div class="col-4 col-offset-1 right-content">
                <h2>Actions sur l'article</h2>
                <input type="checkbox" id="cb1" name="cb" value="" class="form-group">
                <label for="cb1">Activer les commentaires</label><br>
                <input type="checkbox" id="cb2" name="cb"value="" class="form-group">
                <label for="cb2">-----???</label><br>
                <input type="checkbox" id="cb3" name="cb" value="" class="form-group">
                <label for="cb3">Indiqué la date et l'heure de la publication</label><br><br>
                <input type="submit" class="form-group" value="Publier">
                <input type="reset" class="form-group" value="Annuler">
            </div>
            <div class="col-4 col-offset-1 right-content">
                <h2>Catégoriser l'article</h2>
                <input type="checkbox" id="cb1" name="cb" value="" class="form-group">
                <label for="cb1">base</label><br>
                <input type="checkbox" id="cb2" name="cb"value="" class="form-group">
                <label for="cb2">base</label><br>
                <input type="checkbox" id="cb3" name="cb" value="" class="form-group">
                <label for="cb3">base</label><br>
                <input type="checkbox" id="cb1" name="cb" value="" class="form-group">
                <label for="cb1">base</label><br>
                <input type="checkbox" id="cb2" name="cb"value="" class="form-group">
                <label for="cb2">base</label><br>
                <input type="checkbox" id="cb3" name="cb" value="" class="form-group">
                <label for="cb3">base</label><br>
                <input type="checkbox" id="cb1" name="cb" value="" class="form-group">
                <label for="cb1">base</label><br>
                <input type="checkbox" id="cb2" name="cb"value="" class="form-group">
                <label for="cb2">base</label><br>
                <input type="checkbox" id="cb3" name="cb" value="" class="form-group">
                <label for="cb3">base</label><br>
                <input type="checkbox" id="cb1" name="cb" value="" class="form-group">
                <label for="cb1">base</label><br>
                <input type="checkbox" id="cb2" name="cb"value="" class="form-group">
                <label for="cb2">base</label><br>
                <input type="checkbox" id="cb3" name="cb" value="" class="form-group">
                <label for="cb3">base</label><br>
            </div>
        </form>
    </div>
</div>
<?php
include(dirname(__DIR__).'/footer.php');